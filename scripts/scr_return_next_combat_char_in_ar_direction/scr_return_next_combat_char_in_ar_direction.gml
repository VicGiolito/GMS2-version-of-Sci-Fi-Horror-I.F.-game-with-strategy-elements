...
function scr_return_next_combat_char_in_ar_direction(increment_dir, ar_index, original_char_struct_id){
	
	var pc_char_found = false, failsafe_val = 0, failsafe_max = array_length(global.combat_initiative_ar)+1;
	var char_struct_id;
	do {
		...
		//Cap:
		if ar_index < 0 ar_index = array_length(global.combat_initiative_ar)-1;
		else if ar_index >= array_length(global.combat_initiative_ar) ar_index = 0;
		
		char_struct_id = global.combat_initiative_ar[ar_index];
