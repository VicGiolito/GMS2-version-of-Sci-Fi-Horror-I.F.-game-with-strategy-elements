...
function scr_build_char_count_str_from_ar(ar_to_pass){
	...
	for(var i = 0; i < ar_len; i++) {
		...
		if char_struct_id.has_died_bool == false && char_struct_id.has_fled_combat_bool == false {
			...
			if array_length(char_ar) == 0 {
				array_push(char_ar, { char_struct_name: char_struct_id.name, char_count: 1, stunned_bool: char_struct_id.stun_count, collapsed_bool: char_struct_id.unconscious_bool, collapsed_count: char_struct_id.unconscious_count } );
			} else {
	...
	for(var i = 0; i < array_length(char_ar); i++) {
		...
		//If they're stunned, show that:
		if char_ar[i].stunned_bool > 0 {
			var plural_str = "";
			if char_ar[i].stunned_bool > 1 plural_str = "s";
			char_count_str += $" ({char_ar[i].char_count} is stunned for {char_ar[i].stunned_bool} more turn{plural_str})";	
		}
