function scr_define_global_and_con_data(){
	...
	global.help_instructions_str_ar = [
        ...
        "\n'HIDE': Costs 1 action point to hide a character in the current room, using whatever cover they can find. Hidden characters do not automatically trigger combat with enemies in the same room during the start of a new turn, although the player can still allow them to join combat that has already begun, if they desire. When attempting to hide, a stealth-based skill check ensues to determine if this action was successful. Note that the effectiveness of this action is dependent upon the character's stealth stat, the level of cover in the room, and the number of enemies in the room. Generally speaking, the higher the character's stealth stat, and the less enemies there are in a room, the higher the chance of success will be. The exact chance of success is provided to the player before executing this action.\n",
